...
??? book "Chức năng tạo hóa đơn"
    ...
    | **Điều kiện**                   | - Đăng nhập thành công vào tài khoản admin, support.<br>- Nhìn thấy màn hình quản lý hóa đơn. |
...
??? book "Chức năng tìm kiếm bằng ký tự"
    ...
    | **Luồng thao tác chính**        | <br>1. Nhấn vào ô nhập liệu "🔍 Tìm kiếm" nằm ở phía bên trên của màn hình.<br>2. Thực hiện nhập dữ liệu cần tìm:<br>&emsp;- Số lượng ký tự tối thiểu: 1 ký tự.<br>&emsp;- Được phép nhập ký tự đặc biệt.<br>3. Hệ thống sẽ tự động cập nhật dữ liệu trên bảng và biểu đồ thống kê. |
    ...
    | **Trường hợp không có dữ liệu** | Nếu không có dữ liệu được hiển thị trên màn hình hiện tại:<br>&emsp;1. Kiểm tra lại ký tự/ chuỗi ký tự nhập vào.<br>&emsp;2. Liên hệ hỗ trợ trực tiếp. |
...
??? book "Chức năng tìm kiếm bằng bộ lọc thuộc tính"
    ...
    | **Luồng thao tác chính**        | <br>1. Chọn bộ lọc và giá trị lọc tương ứng:<br>&emsp; -`Người Phụ Trách`: danh sách nhân viên có quyền lập hóa đơn và nhận KPI tương ứng hiện ra.<br>&emsp;- `Thời Gian`: bao gồm các lựa chọn `Hôm nay`, `Tuần nay`, `Tuần trước`, `Tháng này`, `Tháng trước`, `Năm nay`, `Tùy chọn`.<br>&emsp;&emsp;*Lưu ý:* `Tùy chọn` *sẽ xuất hiện popup "Lịch", khoảng thời gian chọn sẽ dùng để tìm kiếm*<br>&emsp;- `Loại Hóa Đơn`: lựa chọn loại hóa đơn từ danh sách được hiển thị.<br>&emsp;*(Cọc, Sale, Upsale, Bổ sung học phí, Bổ sung cọc, Renew, Resale)*<br>&emsp;- `Trạng Thái`: lựa chọn trạng thái hóa đơn từ danh sách được hiển thị.<br>&emsp;*(Đợi lớp, Hoàn tiền, Học trước, Hủy đơn, Xác nhận)* |
    ...
    | **Trường hợp không có dữ liệu** | Nếu không có dữ liệu được hiển thị trên màn hình hiện tại:<br>&emsp;1. Kiểm tra giá trị lọc/ các giá trị lọc đã hiển thị tại bộ lọc tương ứng.<br>&emsp;2. Liên hệ hỗ trợ trực tiếp. |
...
??? book "Chức năng tìm kiếm kết hợp ký tự và bộ lọc"
    ...
    | **Luồng thao tác chính**        | <br>Người dùng có thể thực hiện cả hai thao tác sau theo thứ tự tự chọn:<br>1. Tìm kiếm bằng ký tự:<br>&emsp;- Nhấn vào ô nhập liệu “🔍 Tìm kiếm” nằm ở phía trên của màn hình.<br>&emsp;- Thực hiện nhập dữ liệu cần tìm:<br>&emsp;&emsp;- Số lượng ký tự tối thiểu: 1 ký tự.<br>&emsp;&emsp;- Được phép nhập ký tự đặc biệt.<br>2. Tìm kiếm bằng bộ lọc thuộc tính bằng cách chọn bộ lọc và giá trị tương ứng:<br>&emsp; -`Người Phụ Trách`: danh sách nhân viên có quyền lập hóa đơn và nhận KPI tương ứng hiện ra.<br>&emsp;- `Thời Gian`: bao gồm các lựa chọn `Hôm nay`, `Tuần nay`, `Tuần trước`, `Tháng này`, `Tháng trước`, `Năm nay`, `Tùy chọn`.<br>&emsp;&emsp;*Lưu ý:* `Tùy chọn` *sẽ xuất hiện popup "Lịch", khoảng thời gian chọn sẽ dùng để tìm kiếm*<br>&emsp;- `Loại Hóa Đơn`: lựa chọn loại hóa đơn từ danh sách được hiển thị.<br>&emsp;*(Cọc, Sale, Upsale, Bổ sung học phí, Bổ sung cọc, Renew, Resale)*<br>&emsp;- `Trạng Thái`: lựa chọn trạng thái hóa đơn từ danh sách được hiển thị.<br>&emsp;*(Đợi lớp, Hoàn tiền, Học trước, Hủy đơn, Xác nhận)*  |
    | **Điều kiện**                   | - Đăng nhập thành công vào tài khoản admin, support.<br>- Nhìn thấy màn hình quản lý hóa đơn và các bộ lọc tìm kiếm.<br>- Phải có ít nhất một hóa đơn trong toàn bộ hệ thống. |
    ...
    | **Trường hợp không có dữ liệu** | Nếu không có dữ liệu được hiển thị trên màn hình hiện tại:<br>&emsp;1. Kiểm tra ký tự/ chuỗi ký tự tại ô "🔍 Tìm kiếm" và giá trị lọc/ các giá trị lọc đã hiển thị tại bộ lọc tương ứng.<br>&emsp;2. Liên hệ hỗ trợ trực tiếp. |
